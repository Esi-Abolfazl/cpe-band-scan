...
|---|---|---|---|
| `cpe-band-scan scan` | every supported LTE band, then every NR band; ranks by SINR floor with 5G kept, applies the winner with runner-ups as secondaries | 20-30 min | yes, many times |
...
1. **VPN stays on.** Many users run a VPN at all times and it is part of their real connection quality. Never ask them to disconnect it. The scan measures radio quality through the router's LAN API, so the VPN does not affect the numbers; it does affect what the user feels, so ask them to stay on one good, stable VPN server for the whole test window rather than switching mid-run. Each lock change drops the link for ~30 s and the VPN will reconnect each time; that is expected. If the router API is unreachable with the VPN up (full-tunnel without LAN access), ask them to enable the VPN's "allow LAN / local network access" option, not to disconnect.
...
4. **Setup once:** `pipx install cpe-band-scan` (or `pip install --user cpe-band-scan`). Every command below is `cpe-band-scan <command>`, run from the folder that holds `.env`; `cpescan` is a shorter alias for the same thing. The app also has a GUI: `cpe-band-scan ui` opens a local page with the same scan, the same ranked table and a 1-to-10-minute test of any band from the results. Offer it whenever the person would rather click than read a table in chat.
...
8. **Read the result table.** The app prints a markdown table per side (band, rating, lowest/typical/peak SINR, RSRQ, RSRP, 5G kept, carriers) and saves the run. Relay the table to the user as-is. Stability comes from the anchor's SINR **floor** (want > 0 dB) and RSRQ (better than -12 dB); RSRP above -90 dBm hardly matters. Any band that lost the `N` carrier is out. On NSA the NR side follows the LTE anchor; the app still scans NR so the user sees which NR bands are on air, and applies an NR lock only if two or more are live.
9. **The app applies the winner itself** (anchor = best floor, other 5G-keeping bands as secondaries). Confirm with `cpe-band-scan status` that the `N` carrier is still there. Then offer, do not assume: "want me to `test` this lock and the runner-up (2 minutes each) for a firmer pick?" If yes: `cpe-band-scan test`, `cpe-band-scan apply <runner-up> --scell <others>`, `cpe-band-scan test` again, compare floors, re-apply the better one. Report before/after signal lines; `cpe-band-scan runs` / `show <id>` reopens either run later.
10. **Close:** remind the user to change the router password, and that the lock survives reboots (`cpe-band-scan clear` to undo, re-scan after moving or changing SIM/provider).
...
| Judging by peak or by one 25 s window | Noise is ±5 dB; a band can swing -4 to 18 dB. Rank by floor, confirm close calls with `test`. |
